    ...
    private void OnSwapBody()
    {
        ...
        if (gameObject.tag == "Active") //checks if the current player is in control of an active body
        {
            ...
            if(canSwap == true)
            {
                ...
                nextBody.transform.position = new Vector3 (gameObject.transform.position.x , gameObject.transform.position.y + 0.5f , gameObject.transform.position.z); // moves the next body to the current body's position
